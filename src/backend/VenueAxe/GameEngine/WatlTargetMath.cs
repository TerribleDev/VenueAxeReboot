...
/// - Left Killshot (8 pts):  center (-0.380, 0.460), radius 0.073 (Active only when Kill is called)
/// - Right Killshot (8 pts): center (0.380, 0.460), radius 0.073 (Active only when Kill is called)
/// - Drop / Miss (0 pts): outside 1-Ring or dropped axe
/// </summary>
public static class WatlTargetMath
{
    ...
    public static ThrowEvaluation Evaluate(double x, double y, bool isClutchCalled = false)
    {
        ...
        if (distKillshotLeft <= KillshotRadius)
        {
            if (isClutchCalled)
            {
                return new ThrowEvaluation(TargetZone.KillLeft, 8, true, "Killshot Hit! (8 Points)");
            }
            // In WATL, hitting an uncalled killshot is 0 points
            return new ThrowEvaluation(TargetZone.KillLeft, 0, false, "Uncalled Killshot (0 Points)");
        }
        ...
        if (distKillshotRight <= KillshotRadius)
        {
            if (isClutchCalled)
            {
                return new ThrowEvaluation(TargetZone.KillRight, 8, true, "Killshot Hit! (8 Points)");
            }
